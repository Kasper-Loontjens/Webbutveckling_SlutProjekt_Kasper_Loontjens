...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		String API_KEY = "42c76d1530da6ae763ca97ea9913c928";
		...
		// Create a Node list that gets all data of a tag
		NodeList nListClouds = doc.getElementsByTagName("clouds");
		NodeList nListDate = doc.getElementsByTagName("lastupdate");
		NodeList nListWindDirection = doc.getElementsByTagName("direction");

		//Saves tag data as string in the bean
		wBean.setDateStr(nodeConverter(nListDate, wBean, "value"));
		wBean.setCloudsStr(nodeConverter(nListClouds, wBean, "name"));
		wBean.setWindDirectionStr(nodeConverter(nListWindDirection, wBean, "name"));
	}
	
	//Converts Nodelist data into String
	private static String nodeConverter(NodeList nList, weatherBean wBean, String attribute) {
